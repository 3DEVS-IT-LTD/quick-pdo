...
namespace ThreeDevs\QuickPdoIntegrationTests;

use PDO;
use PHPUnit\Framework\TestCase;
use ThreeDevs\templateClasses\User;
use ThreeDevs\QuickPdo\QuickPdo;
...
class QuickPdoIntegrationTest extends TestCase
{
    ...
    {
        $ret = self::$connection->selectClass('SELECT * FROM users', 'ThreeDevs\templateClasses\User');
        self::assertTrue($ret->is_success());
        if($ret->is_success()){
            $data = $ret->get_data();
            $original_data = [
                (new User())->setPkUserId(self::$dataSet[0]['pk_user_id'])->setUserName(self::$dataSet[0]['user_name'])->setUserGender(self::$dataSet[0]['user_gender']),
                (new User())->setPkUserId(self::$dataSet[1]['pk_user_id'])->setUserName(self::$dataSet[1]['user_name'])->setUserGender(self::$dataSet[1]['user_gender']),
                (new User())->setPkUserId(self::$dataSet[2]['pk_user_id'])->setUserName(self::$dataSet[2]['user_name'])->setUserGender(self::$dataSet[2]['user_gender']),
            ];
